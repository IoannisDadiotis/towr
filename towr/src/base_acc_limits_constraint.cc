/* constraint to apply limits to base 6 DoF acceleration */
...
BaseAccLimitsConstraint::BaseAccLimitsConstraint (const std::vector<Vector3d>& acc_max,
                                                  double T_t, double dt,
                                                  const SplineHolder& spline_holder)
    : TimeDiscretizationConstraint(T_t, dt, "base-acc-limits")
{
  ...
  SetRows(GetNumberOfNodes()*k6D);  // each knot has 6 constraints (ax, ay, az, roll, pitch, yaw)
}
